...
public class UserRepositoryImpl implements UserRepository {
    ...
    @Override
    public void getUserById(String id, ResponseCallback<CtbResponse<User>> callback) {
        Call<JSONAPIDocument<User>> call = userService.getUserById(id);
        call.enqueue(new JsonApiResponseMapper<>(objectMapper, callback));
    }

    @Override
    public void getUser(ResponseCallback<CtbResponse<User>> callback) {
        Call<JSONAPIDocument<User>> call = userService.getUser();
        call.enqueue(new JsonApiResponseMapper<>(objectMapper, callback));
    }

    @Override
    public void getUsersList(ResponseCallback<CtbResponse<List<User>>> callback) {
        Call<JSONAPIDocument<List<User>>> call = userService.getUsers();
        call.enqueue(new JsonApiResponseMapper<>(objectMapper, callback));
    }

    @Override
    public void createUser(NewUser newUser, ResponseCallback<CtbResponse<User>> callback) {
